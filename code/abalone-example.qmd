...
H~0~: **There is no effect of pH on the predicted abalone growth (measured in change in shell surface area per day, mm^-2 d^-1^**  

H~A~: **There is an effect of pH on the predicted abalone growth (measured in change in shell surface area per day, mm^-2^ d^-1^**
...
# create object abalone_clean from original dataset
abalone_clean <- abalone |>  
  # remove underscores and make lowercase
  clean_names() |>
  # keep only columns about mean pH and growth
...
# base layer: ggplot
ggplot(data = abalone_clean, # x-axis: mean_ph 
       aes(x = mean_ph, # y-axis: surface area change
           y = change_in_area)) +
  # first layer: geom points to represent individual observations
  geom_point(size = 4,
             stroke = 1,
             fill = "firebrick4",
             shape = 21)
...
**The missing value comes from a pair of tanks where the pH sensor malfunctioned. Therefore, there is no pH reading for the tanks**  
...
"x", "predicted", "std.error", "conf.low", "conf.high", "group" column for now 
...
This is a ggeffects object but it is also a dataframe.  
...
At pH = 8, the models predicts abalone shell growth of 5.36 [95% CI: 5.08, 5.64] m^2^ d^-1^
